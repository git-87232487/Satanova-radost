...
$container.find('.navlinks').each(function(){var $this=$(this),$left=$this.children().not('.rightside'),$right=$this.children('.rightside');if($left.length!==1||!$right.length){return;}
...
check();$(window).resize(check);});var selector='.linklist:not(.navlinks, [data-skip-responsive]),'+
...
jQuery(function($){'use strict';$('#phpbb.nojs').toggleClass('nojs hasjs');$('#phpbb').toggleClass('hastouch',phpbb.isTouch);$('#phpbb.hastouch').removeClass('notouch');$('form[data-focus]:first').each(function(){$('#'+this.getAttribute('data-focus')).focus();});parseDocument($('body'));});jQuery(function($){'use strict';$("li.header").append('<div class="triangle"></div>');});$("#forumlist-collapse").collapse({query:'.collapse-trigger',persist:true,open:function(){this.slideDown(100);},close:function(){this.slideUp(100);},});